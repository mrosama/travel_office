...
use App\Http\Requests\Travel_officesRequest;
use App\Travel_office;
use App\Travel_officeFax;
use App\Travel_officeEmail;
use App\Travel_officePhone;
use App\Travel_officeMobile;
use App\Client;
use App\User;
use Mail;
use Redirect;

class Travel_officesController extends Controller {

    public function index() {
        $all_office = Travel_office::all();
        return view('admin.travel_office.index', compact('all_office'));
    }

    public function create() {
        $all_country = Country::lists('name', 'code');
        return view('admin.travel_office.create', compact('all_country'));
    }

// public function store(Travel_officesRequest $request, Client $upload) {
    public function store(Request $request, Client $upload) {
        ...
        $data['logo'] = $logo;
        $new_office = Travel_office::create($data);
        $office_id = $new_office->id;
        if (isset($request['emails'])) {
            foreach ($request['emails'] as $key => $val) {
                $data['email'] = $val;

                $data['travel_officeId'] = $office_id;
                Travel_officeEmail::create($data);
            }
        }
        if (isset($request['mobiles'])) {
            foreach ($request['mobiles'] as $key => $val) {
                $data['number'] = $val;
                $data['travel_officeId'] = $office_id;
                Travel_officeMobile::create($data);
            }
        }
        if (isset($request['phones'])) {
            foreach ($request['phones'] as $key => $val) {
                $data['number'] = $val;
                $data['travel_officeId'] = $office_id;
                Travel_officePhone::create($data);
            }
        }
    ...
    public function edit($id) {
        $office = Travel_office::find($id);
        $all_country = Country::get();
        $all_city = City::where('country_code', $office->country)->get();
        $emails = Travel_officeEmail::where('travel_officeId', $id)->get();
        $mobiles = Travel_officeMobile::where('travel_officeId', $id)->get();
        $phones = Travel_officePhone::where('travel_officeId', $id)->get();
        $faxs = Travel_officeFax::where('travel_officeId', $id)->get();
        return view('admin.travel_office.edit', compact('office', 'all_country', 'all_city', 'emails', 'mobiles', 'phones', 'faxs'));
    }

   // public function update(Travel_officesRequest $request, $id, Client $upload) {
    public function update(Request $request, $id, Client $upload) {

        $data = $request->except('password_confirmation');
        $office = Travel_office::find($id);
        $oldPassword = $office->password;
        ...
        if ($request->logo != null) {
            \File::delete(public_path() . $office->logo);
            $logo = $upload->uploadFile($request->logo);
        } else {
            $logo = $office->logo;
        }
        ...
        Travel_office::find($id)->update($data);

        if (isset($request['emails'])) {
            Travel_officeEmail::where('travel_officeId', $id)->delete();
            foreach ($request['emails'] as $key => $val) {
                $data['email'] = $val;
                $data['travel_officeId'] = $id;
                Travel_officeEmail::create($data);
            }
        }

        if (isset($request['mobiles'])) {
            Travel_officeMobile::where('travel_officeId', $id)->delete();
            foreach ($request['mobiles'] as $key => $val) {
                $data['number'] = $val;
                $data['travel_officeId'] = $id;
                Travel_officeMobile::create($data);
        ...
        if (isset($request['phones'])) {
            Travel_officePhone::where('travel_officeId', $id)->delete();
            foreach ($request['phones'] as $key => $val) {
                $data['number'] = $val;
                $data['travel_officeId'] = $id;
                Travel_officePhone::create($data);
        ...
        if (isset($request['faxs'])) {
            Travel_officeFax::where('travel_officeId', $id)->delete();
            foreach ($request['faxs'] as $key => $val) {
                $data['fax'] = $val;
                $data['travel_officeId'] = $id;
                Travel_officeFax::create($data);
    ...
    public function destroy($id) {
        Travel_office::find($id)->destroy($id);
        Travel_officeEmail::where('travel_officeId', $id)->delete();
        Travel_officeMobile::where('travel_officeId', $id)->delete();
        Travel_officePhone::where('travel_officeId', $id)->delete();
        Travel_officeFax::where('travel_officeId', $id)->delete();
        return Redirect::back()->with('success', 'لقد تمت عملية الحذف بنجاخ');
    }

    public function sendEmail($id) {
        $data['office'] = Travel_office::find($id);
        return view('admin.travel_office.sendEmail', $data);
    }

    public function doSendEmail(Request $request, $id) {
        $this->validate($request, [
            'msg_title' => 'required',
            'msg_details' => 'required'
        ]);
        $office_email = Travel_office::find($id)->email;
// Send Email To Office
        $title = $request->msg_title;
        $msg = $request->msg_details;
        Mail::send('admin.emails.officeSendEmail', array('msg_data' => $msg), function($message) use($office_email, $title) {
            $message->to($office_email)->subject($title);
        });
    ...
    public function sendSMS($id) {
        $data['office'] = Travel_office::find($id);
        return view('admin.travel_office.sendSMS', $data);
    }

    public function doSendSMS(Request $request, $id) {
        $this->validate($request, [
            'msg_details' => 'required'
        ]);

        $office_mobile = Travel_office::find($id)->mobile;

//Send Sms To Client
        $number = $office_mobile;
        $msg = convertToUnicode($request->msg_details);
        ...
        if ($ret == 1)
            ...
            return Redirect::back()->with('error', 'حدث خطأ اثناء الارسال يرجى مراجعة المطور');
    }
